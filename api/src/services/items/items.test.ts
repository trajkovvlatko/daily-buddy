...
describe('items', () => {
  ...
  scenario('creates a item', async (scenario: StandardScenario) => {
    const result = await createItem({
      input: {
        name: 'String',
        drawerId: scenario.item.two.drawerId,
        userId: scenario.item.two.userId,
        colorId: scenario.item.two.colorId,
        itemTypeId: scenario.item.two.itemTypeId,
      },
    });

    expect(result.name).toEqual('String');
    expect(result.drawerId).toEqual(scenario.item.two.drawerId);
    expect(result.userId).toEqual(scenario.item.two.userId);
    expect(result.colorId).toEqual(scenario.item.two.colorId);
    expect(result.itemTypeId).toEqual(scenario.item.two.itemTypeId);
  });

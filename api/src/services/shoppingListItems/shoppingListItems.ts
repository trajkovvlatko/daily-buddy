...
const validateAccess = async ({ userId, shoppingListId }: { userId: number; shoppingListId: number }) => {
  ...
  await db.shoppingList.findFirstOrThrow({
    where: {
      OR: [{ id: shoppingListId, userId }, { id: accessibleId }],
    },
  });
};

export const updateShoppingListItem: MutationResolvers['updateShoppingListItem'] = async (
  { id, input },
  { context }
) => {
...
export const deleteAllBoughtItems: MutationResolvers['deleteAllBoughtItems'] = async (
  { shoppingListId },
  { context }
) => {
  const userId = context.currentUser['id'];

...
const ShareForm = ({ id, type, emails = [] }: { id: number, type: string, emails?: string[] }) => {
  ...
  const share = async () => {
    const emails = inputRef.current.value.split(",").filter(Boolean).map((email) => email.trim())
  ...
  return <div>
    ...
    <div className="flex items-baseline">
      <input type='text' className='rw-input mr-3' placeholder='Comma-separate emails' defaultValue={emails.join(", ")} ref={inputRef} />
      <button className="green-button" onClick={share}>Share</button>
    </div>
  </div>;
};
